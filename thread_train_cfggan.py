...
def check_args_(opt):
  ...
  #  opt.optim_type=RMSprop_str; opt.optim_eps=1e-18; opt.optim_a1=0.95; opt.optim_a2=-1
   opt.optim_type=Adam_str; opt.optim_eps=-1; opt.optim_a1=-1; opt.optim_a2=-1
...
def main(args):
   ...
   if size > 1:
        ...
        for rank in range(size):
            ...
            p = Process(target=init_processes, args=(global_rank, global_size, thread_cfggan_train, opt))
            p.start()
            processes.append(p)
        ...
        init_processes(0, size, thread_cfggan_train, opt)   
  #  thread_cfggan_train(opt)
