...
def is_time_to_fid(opt, stage):
   return opt.save_interval > 0 and (stage+1)%opt.save_interval == 0 or is_last(opt, stage)
def change_gp(opt,gptype=0):
   if gptype == 2:
     opt.gptype = 2
     opt.scale = 0.1
   elif gptype == 0:
...
def cfggan(opt, d_config, g_config, z_gen, loader,fromfile=None,saved=None,z_y_gen = None,
   ...
   for stage in range(begin,opt.num_stages):   
      ...
      if is_time_to_fid(opt, stage):
         fidvalue=generate_compute_fid(opt, ddg, stage)
         if fidvalue < 3:
           print('change gp')
           gptype = 2
           if gptype == 2:
...
def write_real_num(opt, loader,num=1200):
   ## appoint the num = 1000
   timeLog('write_real_num: ... ')
   dir = 'real_mnist'
   if not os.path.exists(dir):
      os.mkdir(dir)
   print('len of loader {}'.format(len(loader)))
...
class DDG:
   def __init__(self, opt, d_config, g_config, z_gen, optim_config, from_file=None):
      ...
      # self.simple_net,self.simple_params = simple_mlp(1,1,True)
      # self.logger = Logger('./logs/' + str(opt.gen)+'-' +str(opt.cfg_eta)+ "/")
      if optim_config is not None:
         self.d_optimizer = optim_config.create_optimizer(self.d_params,'D')
         # self.simple_optimizer = optim_config.create_optimizer(self.simple_params,'D')
   ...
   def load(self,  d):
      ...
      # self.d_optimizer = d['d_optimizer']  
   ...
   def generate(self, num_gen, t=-1, do_return_z=False, batch_size=-1):
      ...
      while num_gened < num_gen:
         ...
         with torch.no_grad():
            ...
            #   gy.sample_()
              fake = self.g_net(cast(z),cast(gy), self.g_params, is_train)
            else:
              z,gy = self.z_gen(num)
         ...
         for t0 in range(t):
            ...
            if self.num_class is not None:
              d_out = self.d_net(fake, self.get_d_params(t0), cast(gy))
            else:
            ...
            # test_data = math.exp(d_out.mean())
            # print(test_data)
            # a = self.simple_net(d_out,self.simple_params,True)
            # a = a[:,:,None,None]
            fake.data += self.cfg_eta * fake.grad.data
            # print(a.shape)
            # print(fake.grad.data.shape)
            # fake.data += a* fake.grad.data
   ...
   def icfg(self, loader, iter, d_loss, cfg_U):   
      ...
      is_train = True
      print('gptypesss....{}'.format(self.gptype))
      for t in range(self.num_D()):
         ...
         for upd in range(cfg_U):
            ...
            #   print(sample[1])
            #   print(gy)
              d_out_real = self.d_net(cast(sample[0]), self.d_params, y=cast(sample[1]))
              d_out_fake = self.d_net(cast(fake), self.d_params, cast(gy))
            else:
            ...
            # print('grad',self.simple_params['fc.weight'])
            # print('grad',self.simple_params['fc1.weight'])  
            # # print(self.simple_params['fc.weight'].grad)
   ...
   def initialize_G(self, g_loss, cfg_N): 
      ...
      if self.num_class is not None:
         ...
         # gy.sample_()
         # print(gy)
         g_out = self.g_net(cast(z),cast(gy), self.g_params, False)
         z_dim = z.size(1)
      else:
         z,gy = self.z_gen(1)
   ...
   def _approximate(self, loader, g_loss): 
      ...
      optimizer = self.optim_config.create_optimizer(g_params,'G')
      ...
      for epoch in range(self.optim_config.cfg_x_epo):
         ...
         for sample in loader:
            ...
            #   print('z.shape{}'.format(z.shape))
              fake = self.g_net(cast(z),cast(gy), g_params, is_train)
            else:
...
def generate_compute_fid(opt, ddg, stage='',l='1'):
   gen_dir_dict = {}
   swap_num_gen=opt.num_gen
   swap_gen_nrow=opt.gen_nrow
   swap_gen=opt.gen
   opt.gen_nrow = -1
   opt.num_gen = 1000
   gen_dir_dict['dataset'] = opt.dataset
   gen_dir_dict['lr'] = opt.lr
   gen_dir_dict['batch_size'] = opt.batch_size
   gen_dir_dict['cfg_eta'] = opt.cfg_eta
   gen_dir_dict['model'] = opt.model
   gen_dir_dict['cfg_T'] = opt.cfg_T
   
   dir = 'gen_'+add_sep_for_dict('_', **gen_dir_dict)+str(stage)
   opt.gen = dir+'/generate'
   # print(opt.gen)
   # opt.gen = 'gen'+'_'+opt.dataset+'_'+opt.lr+'_'+opt.batch_size+'_'+opt.cfg_eta+'_'+opt.model+'_'+opt.cfg_T+'_'+stage+'/'+'result'
   ...
   fid_value = fid_score.calculate_fid_given_paths([dir,opt.real_path],
                                          50,
                                          'cuda:0',
                                          2048,
                                          8)
   print('fid_value{},stages{}'.format(fid_value,stage))
   opt.num_gen =  swap_num_gen
   opt.gen_nrow =  swap_gen_nrow
   opt.gen =  swap_gen
   return fid_value
