...
def cfggan(opt, d_config, g_config, z_gen, loader,fromfile=None,saved=None,z_y_gen = None,
   ...
   for stage in range(begin,opt.num_stages):   
      ...
      g_loss_v=ddg.approximate(g_loss, opt.cfg_N,loader,iterator)         
...
class DDG:
   ...
   #----------------------------------------------------------      
   def generate(self, num_gen, t=-1, do_return_z=False, batch_size=-1,gy_=None):
      assert num_gen > 0
      if t < 0:
         t = self.num_D()
      if batch_size <= 0:
      ...
      while num_gened < num_gen:
         ...
         with torch.no_grad():
            if self.num_class is not None:
              z,gy = self.z_gen(num,self.num_class)
            #   print(gy)
              if gy_ is not None:
                gy = gy_
                print(gy)
         ...
         for t0 in range(t):
            ...
            # print(gy.requires_grad)
            if self.num_class is not None:
              d_out = self.d_net(fake, cast(gy) ,self.get_d_params(t0),True)
            else:
            ...
            # data1 =  torch.cat([gy.grad.data.repeat(1,3), gy.grad.data[:,:2]], 1)
            # data1=data1.view(data1.shape[0],1,1,data1.shape[1])
            # data1= data1.repeat(1,1,32,1)
            # fake.data += self.cfg_eta * (fake.grad.data+data1.cuda())
            fake.data += self.cfg_eta * fake.grad.data
            # print(data1.shape)
            # gy.data+=self.cfg_eta*gy.grad.data
   ...
   def icfg(self, loader, iter, d_loss, cfg_U):   
      ...
      for t in range(self.num_D()):
         ...
         for upd in range(cfg_U):
            ...
            self.dy = sample[1]
            # print(self.dy)
            one_hot_labels = torch.FloatTensor(num, self.num_class)
            one_hot_labels.resize_(num, self.num_class).zero_()
            one_hot_labels.scatter_(1, self.dy.view(num,1), 1)
            ...
            if self.num_class is not None:
              d_out_real = self.d_net(cast(sample[0]),cast(one_hot_labels),self.d_params,is_train)
              d_out_fake = self.d_net(cast(fake), cast(gy), self.d_params,is_train)
            else:
   ...
   def initialize_G(self, g_loss, cfg_N): 
      ...
      if self.num_class is not None:
         z,gy = self.z_gen(1,self.num_class)
         # print(gy.shape)
         # print(gy)
      ...
      if self.num_class is not None:
        gys = torch.Tensor(cfg_N,self.num_class)
      with torch.no_grad():      
   ...
   def approximate(self, g_loss, cfg_N,loader,iterator): 
      ...
      while num_gened < cfg_N:
         ...
         one_hot_labels = torch.FloatTensor(num, self.num_class)
         one_hot_labels.resize_(num, self.num_class).zero_()
         one_hot_labels.scatter_(1, sample[1].view(num,1), 1)
         if sample_list is None:
            sz = [cfg_N] + list(one_hot_labels.size())[1:]
            sample_list = torch.Tensor(torch.Size(sz), device=torch.device('cpu')) 
         sample_list[num_gened:num_gened+num]=one_hot_labels
         num_gened += num 
      if self.num_class is not None:
